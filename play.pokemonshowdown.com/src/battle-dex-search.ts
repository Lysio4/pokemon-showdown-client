...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected formatType: 'doubles' | 'bdsp' | 'bdspdoubles' | 'rs' | 'frlg' | 'bw1' | 'letsgo' | 'metronome' | 'natdex' |
		...
		'svdlc1natdex' | 'stadium' | 'lc' | 'champions' | 'agoldenexperience' | 'touhoumons' | null = null;
	isDoubles = false;
	...
	constructor(searchType: T, format = '' as ID, speciesOrSet: ID | Dex.PokemonSet = '' as ID) {
		...
		if ((format.includes('nationaldex') || format.startsWith('nd') || format.includes('natdex')) && !format.includes('champions')) {
	...
	protected canLearn(speciesid: ID, moveid: ID) {
		...
		if ((this.formatType === 'natdex' || this.formatType === 'agoldenexperience' || this.formatType.includes('agoldenexperience') || this.formatType === 'toho' || this.formatType === 'touhoumons') &&
		...
		if (
			...
			(this.dex.gen === 9 && this.formatType !== 'natdex' && this.formatType !== 'agoldenexperience' && this.formatType !== 'toho' && this.formatType !== 'touhoumons')
		) {
			if (gen === 9) {
				genChar = 'a';
			} else if (gen === 8) {
...
class BattlePokemonSearch extends BattleTypedSearch<'pokemon'> {
	...
	getBaseResults(): SearchRow[] {
		...
		if (format === 'ubers' || format === 'uber' || format === 'ubersuu' || format === '4v4doublesuu' || format === 'nationaldexdoubles' || format === 'agoldenexperiencedoubles') {
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	static readonly GOOD_WEAK_MOVES = [
		'accelerock', 'acrobatics', 'aquacutter', 'aramisdagger', 'avalanche', 'barbbarrage', 'bonemerang', 'bouncybubble', 'bulletpunch', 'buzzybuzz', 'ceaselessedge', 'circlethrow', 'clearsmog', 'doubleironbash', 'dragondarts', 'dragontail', 'drainingkiss', 'endeavor', 'facade', 'firefang', 'flamingsphere', 'flipturn', 'flowertrick', 'freezedry', 'frustration', 'geargrind', 'gigadrain', 'grassknot', 'gyroball', 'icefang', 'iceshard', 'iciclespear', 'infernalparade', 'knockoff', 'landslide', 'lastrespects', 'lowkick', 'machpunch', 'mortalspin', 'mysticalpower', 'naturesmadness', 'nightshade', 'nuzzle', 'pikapapow', 'populationbomb', 'psychocut', 'psyshieldbash', 'pursuit', 'quickattack', 'ragefist', 'rapidspin', 'return', 'rockblast', 'roguewave', 'ruination', 'saltcure', 'scaredyshell', 'scorchingsands', 'seismictoss', 'shadowclaw', 'shadowsneak', 'sizzlyslide', 'stoneaxe', 'storedpower', 'stormthrow', 'suckerpunch', 'superfang', 'surgingstrikes', 'sweetheart', 'tachyoncutter', 'tailslap', 'thunderclap', 'tripleaxel', 'tripledive', 'twinbeam', 'uturn', 'vacuumwave', 'veeveevolley', 'voltswitch', 'watershuriken', 'weatherball',
	] as ID[] as readonly ID[];
	...
	getBaseResults() {
		...
		const regionBornLegality = dex.gen >= 6 &&
			(/^battle(spot|stadium|festival)/.test(format) || format.startsWith('bss') ||
				format.startsWith('vgc') || (dex.gen === 9 && this.formatType !== 'natdex' && this.formatType !== 'agoldenexperience' && this.formatType !== 'toho'));
		...
		while (learnsetid) {
			...
			if (learnset) {
				for (let moveid in learnset) {
					...
					if (this.formatType !== 'natdex' && this.formatType !== 'agoldenexperience' && !this.formatType.includes('agoldenexperience') && this.formatType !== 'touhoumons' && this.formatType !== 'toho' && move.isNonstandard === "Past") {
						continue;
					}
					if (
