...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected canLearn(speciesid: ID, moveid: ID) {
		...
		if ((this.formatType?.includes('natdex') || this.formatType === 'agoldenexperience' || this.formatType.includes('agoldenexperience') || this.formatType === 'toho' || this.formatType === 'touhoumons') &&
		...
		if (
			...
			(this.dex.gen === 9 && !this.formatType?.includes('natdex') && this.formatType !== 'agoldenexperience' && this.formatType !== 'toho' && this.formatType !== 'touhoumons')
		) {
			if (gen === 9) {
				genChar = 'a';
			} else if (gen === 8) {
...
class BattlePokemonSearch extends BattleTypedSearch<'pokemon'> {
	...
	getBaseResults(): SearchRow[] {
		...
		if (
		...
		} else if (format === 'ou' || (format.includes('ou') && !format.includes('touhoumons') && !format.includes('doubles'))) tierSet = tierSet.slice(slices.OU);
		...
		else if (format === 'agoldenexperiencedoubles' || (format.includes('doubles') && !format.includes('uu') && !format.includes('uber'))) tierSet = tierSet.slice(slices.DOU);
			...
			tierSet = tierSet.slice(0, slices.Toho || slices.AG || slices.Uber).concat(tierSet.slice(slices.OU));
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	private moveIsNotUseless(id: ID, species: Dex.Species, moves: string[], set: Dex.PokemonSet | null) {
		...
		case 'solarbeam': case 'solarblade':
			return ['desolateland', 'drought', 'chlorophyll', 'orichalcumpulse', 'solarenergy', 'sandsoftime'].includes(abilityid) || itemid === 'powerherb';
	...
	static readonly GOOD_STATUS_MOVES = [
		'acidarmor', 'agility', 'aromatherapy', 'auroraveil', 'autotomize', 'awakening', 'banefulbunker', 'batonpass', 'bellydrum', 'bulkup', 'burningbulwark', 'calmmind', 'chillblain', 'chillyreception', 'clangoroussoul', 'coil', 'cottonguard', 'courtchange', 'curse', 'defog', 'destinybond', 'detect', 'disable', 'dragondance', 'encore', 'extremeevoboost', 'filletaway', 'frostbite', 'geomancy', 'glare', 'goatup', 'haze', 'healbell', 'healingwish', 'healorder', 'heartswap', 'highwater', 'honeclaws', 'kingsshield', 'junglehealing', 'leechseed', 'lifedew', 'lightscreen', 'lovelykiss', 'lunarblessing', 'lunardance', 'magiccoat', 'maxguard', 'memento', 'milkdrink', 'mistystep', 'moonlight', 'morningsun', 'musclecare', 'nastyplot', 'naturesmadness', 'ningencry', 'noretreat', 'obstruct', 'painsplit', 'partingshot', 'perishsong', 'protect', 'quiverdance', 'recover', 'reflect', 'reflecttype', 'rest', 'revivalblessing', 'roar', 'rockpolish', 'roost', 'shedtail', 'shellsmash', 'shiftgear', 'shoreup', 'silktrap', 'slackoff', 'sleeppowder', 'sleeptalk', 'softboiled', 'spikes', 'spikyshield', 'spore', 'stealthrock', 'stickyweb', 'strengthsap', 'substitute', 'switcheroo', 'swordsdance', 'synthesis', 'tailglow', 'tailwind', 'taunt', 'thunderwave', 'tidyup', 'toxic', 'transform', 'trick', 'victorydance', 'whirlwind', 'willowisp', 'wish', 'yawn',
	] as ID[] as readonly ID[];
	static readonly GOOD_WEAK_MOVES = [
		'accelerock', 'acrobatics', 'aquacutter', 'avalanche', 'barbbarrage', 'bonemerang', 'brainblast', 'bouncybubble', 'bulletpunch', 'buzzybuzz', 'ceaselessedge', 'circlethrow', 'clearsmog', 'cruelfeather', 'doubleironbash', 'dragondarts', 'dragontail', 'drainingkiss', 'endeavor', 'facade', 'firefang', 'flamingsphere', 'flipturn', 'flowertrick', 'freezedry', 'frustration', 'fulldevotion', 'futuredoom', 'geargrind', 'gigadrain', 'goodfishing', 'grassknot', 'gyroball', 'houndshowl', 'icefang', 'iceshard', 'iciclespear', 'infernalparade', 'intrepidcrash', 'knockoff', 'landslide', 'lastrespects', 'lowkick', 'machpunch', 'mortalspin', 'mysticalpower', 'naturesmadness', 'nightshade', 'nuzzle', 'pikapapow', 'poisonwhip', 'populationbomb', 'psychocut', 'psyshieldbash', 'pursuit', 'quickattack', 'ragefist', 'rapidspin', 'return', 'rockblast', 'roguewave', 'ruination', 'saltcure', 'scorchingsands', 'seismictoss', 'shadowclaw', 'shadowsneak', 'sizzlyslide', 'sneakyassault', 'sonicboom', 'stoneaxe', 'stonesurge', 'storedpower', 'stormthrow', 'suckerpunch', 'superfang', 'surgingstrikes', 'sweetheart', 'tachyoncutter', 'tailslap', 'thunderclap', 'tripleaxel', 'tripledive', 'twinbeam', 'uturn', 'vacuumwave', 'veeveevolley', 'voltswitch', 'watershuriken', 'weatherball',
	] as ID[] as readonly ID[];
	...
	getBaseResults() {
		...
		const regionBornLegality = dex.gen >= 6 &&
			(/^battle(spot|stadium|festival)/.test(format) || format.startsWith('bss') ||
				format.startsWith('vgc') || (dex.gen === 9 && !this.formatType?.includes('natdex') && this.formatType !== 'agoldenexperience' && this.formatType !== 'toho'));
		...
		while (learnsetid) {
			...
			if (learnset) {
				for (let moveid in learnset) {
					...
					if (!this.formatType?.includes('natdex') && this.formatType !== 'agoldenexperience' && !this.formatType.includes('agoldenexperience') && this.formatType !== 'touhoumons' && this.formatType !== 'toho' && move.isNonstandard === "Past") {
						continue;
					}
					if (
		...
		if (sketch || isHackmons) {
			...
			for (let id in BattleMovedex) {
				...
				if (sketch) {
					...
					if (move.isNonstandard === 'Past' && this.formatType !== 'natdex' && this.formatType !== 'agoldenexperience') continue;
					if (move.isNonstandard === 'LGPE' && this.formatType !== 'letsgo' && this.formatType !== 'agoldenexperience') continue;
					moves.push(move.id);
				}
			}
		}
